...
class QualityValidator:
    ...
    def _validate_single_file(self, file_path: Path) -> list[str]:
        ...
        try:
            ...
            has_pytest_markers = any(
                decorator.id == "pytest.mark"
            ...
            if not has_pytest_markers:
                issues.append(
                    "No pytest markers found - consider adding @pytest.mark annotations",
                )
            ...
            for func in test_functions:
                if not func.name.startswith("test_"):
                    issues.append(
                        f"Test function {func.name} doesn't follow naming convention",
                    )
            ...
            has_pytest_import = any(
                (
                    ...
                    and any(alias.name == "pytest" for alias in imp.names)
                )
                or (isinstance(imp, ast.ImportFrom) and imp.module == "pytest")
                for imp in imports
            )
    ...
    def validate_test_markers(self) -> list[str]:
        ...
        if pytest_ini.exists():
            with Path(pytest_ini).open(encoding="utf-8") as f:
                content = f.read()
    ...
    def validate_fixture_usage(self) -> list[str]:
        ...
        if conftest_file.exists():
            try:
                with Path(conftest_file).open(encoding="utf-8") as f:
                    content = f.read()
    ...
    def generate_quality_report(self) -> dict[str, Any]:
        ...
        total_issues = sum(
            len(issues) for issues in report.values() if isinstance(issues, list)
        )
        report["summary"] = {
    ...
    def analyze_component_coverage(self) -> dict[str, Any]:
        ...
        for src_module in src_modules:
            ...
            corresponding_tests = [
                ...
                or any(part in test_file.name for part in relative_path.parts)
            ]
...
class TestQualityValidation:
    ...
    def test_quality_report_generation(self) -> None:
        ...
        assert report["summary"]["overall_quality"] in {
            "EXCELLENT",
            "GOOD",
            "NEEDS_IMPROVEMENT",
            "POOR",
