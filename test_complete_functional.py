...
async def test_oracle_wms_connection() -> FlextResult[Any]:
    ...
    try:
        ...
        logger.info(f"  📋 Discovered entities: {health_data.get('discovered_entities')}")
...
async def test_data_extraction() -> FlextResult[dict[str, Any]]:
    ...
    try:
        ...
        for entity in test_entities:
            ...
            try:
                ...
                if data_result.is_success:
                    ...
                    logger.warning(f"  ❌ Failed to extract from {entity}: {data_result.error}")
        ...
        total_records = sum(result.get('record_count', 0) for result in extraction_results.values())
...
async def main() -> int:
    ...
    logger.info("   This test will perform real Oracle WMS operations using credentials from .env")
    ...
    try:
        ...
        logger.info(f"  ✅ Schema Discovery: SUCCESS ({len(discovery_result.data)} entities)")
        logger.info(f"  ✅ Data Extraction: SUCCESS ({extraction_data['successful_extractions']}/{extraction_data['entities_tested']} entities)")
