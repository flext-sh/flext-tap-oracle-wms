    ...
    def discover_streams(self) -> list[Stream]:
        ...
        for entity_name, schema in schemas.items():
            try:
                ...
                logger.exception(
                    "Unexpected error creating stream for entity %s",
                    entity_name,
                )
                if self._monitor:
                    self._monitor.metrics.record_counter(
            ...
            if replication_key in {"mod_ts", "create_ts", "last_updated_ts"}:
                ...
                if isinstance(bookmark_value, str):
                    try:
                        ...
                        logger.info(
                            "🔄 Incremental filter for %s: %s >= %s (bookmark: %s, overlap: %smin)",
                            entity_name,
                            replication_key,
                            filter_dt.isoformat(),
                            bookmark_value,
                        ...
                        logger.warning(
                            "Invalid bookmark timestamp for %s: %s",
                            entity_name,
                            e,
                        )
    ...
    def apply_full_sync_filters(
        ...
        if strategy == "id_based_resume" and resume_context:
            ...
            if resume_context.get("has_existing_data") and resume_context.get(
                ...
                logger.info(
                    "🔄 Full sync resume for %s: id < %s (min_id: %s) with -id ordering",
                    entity_name,
                    filter_threshold,
                    min_id,
                )
                ...
                params["ordering"] = "-id"
                logger.info(
                    "🔄 Full sync for %s: -id ordering (no existing data)",
                    entity_name,
                )
            ...
            logger.info("🔄 Full sync for %s: default ordering", entity_name)
