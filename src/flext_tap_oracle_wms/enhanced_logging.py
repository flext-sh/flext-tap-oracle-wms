...
def setup_enhanced_logging(
    module_name: str = "flext_tap_oracle_wms",
    level: str | int = "TRACE",
...
try:
    from .logging import get_singer_logger, trace_singer_operation
    ENTERPRISE_LOGGING_AVAILABLE = True
except ImportError:
    ENTERPRISE_LOGGING_AVAILABLE = False

# CLI integration function with enterprise logging support
def setup_cli_logging(config: dict[str, Any]) -> TraceLogger:
    """Setup logging specifically for CLI usage with enterprise integration.
    ...
    # Enterprise logging integration if available
    if ENTERPRISE_LOGGING_AVAILABLE:
        enterprise_logger = get_singer_logger("cli_enhanced_logging")
        trace_singer_operation(
            enterprise_logger,
            operation="cli_logging_setup",
            timing_ms=0.0,
            log_level=log_level,
            file_logging=log_file is not None,
            config_items=len(config),
        )
        enterprise_logger.trace("🎯 Enhanced logging integrated with enterprise system")
